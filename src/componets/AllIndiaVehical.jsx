...
function VehicleTable() {
  ...
  return (
    <div className="overflow-hidden">
      ...
      <div className="fixed top-20 left-0 w-full bg-gray-100 shadow-md z-10">
        <div className="flex flex-wrap items-center justify-between px-4 py-3 space-y-4 md:space-y-0">
          {/* Back Button */}
          <button
            onClick={() => navigate("/dashboard")}
            className="flex items-center text-blue-800 hover:text-blue-600"
          >
            <FaArrowLeft className="text-2xl mr-2" />
            <span className="text-lg font-bold sm:text-xl">Back</span>
          </button>

          {/* Title */}
          <div className="flex items-center text-blue-800 hover:text-blue-600">
            <span className="text-lg sm:text-2xl font-bold text-center">
              All India Vehicle Data
            </span>
          </div>

          {/* Actions: Search, Export to CSV, Show All */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-4 sm:space-y-0 w-full sm:w-auto">
            <input
              type="text"
              placeholder="Search by Vendor, Vehicle No, or Owner No"
              className="border p-2 w-full sm:max-w-md rounded focus:outline-none"
              value={searchTerm}
              onChange={handleSearchChange}
            />
            <CSVLink
      ...
      {/* DataTable Section */}
      <div className="pt-80 md:pt-40 mt-4 p-4 overflow-x-auto">
        {loading || showAllLoading ? (
          <div className="text-center text-blue-800 font-bold">Loading...</div>
        ) : (
          <DataTable
